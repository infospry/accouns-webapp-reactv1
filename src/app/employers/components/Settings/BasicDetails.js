import React from 'react'

export default function BasicDetails() {
    return (
        <>
            <div class="panel-group">
                <div class="panel panel-primary  bdr-1">
                    <div class="panel-body pt-0 pb-0">
                        <div class="row">
                            <div class="col-sm-12 formbg p-0">
                                <div class="avt_bg d-flex justify-content-between align-items-center">
                                    <h3 class="mb-0"><b>Basic
                                        Information</b>
                                    </h3>




                                </div>
                                <div class="p-4 bdr_b1 View_General_info">
                                    <div class="row justify-content-start">
                                        <div class="col-12 col-lg-6">
                                            <h4>Company Name
                                            </h4>
                                            <p class="mb-3 profileView">
                                                <span id="spanProName" class="col-black font-16 font-600">
                                                    IST Pvt Ltd4"</span>
                                            </p> <div class="tools">
                                                <h4>Profile completion(%)
                                                </h4>

                                                <div class="tools-progres">

                                                    <div class="progres pb-0 mb-3 mt-1">
                                                        <p>95%</p>
                                                        <div class="prog">
                                                            <span class="bg-lnir" style={{ width: "95%" }}></span>
                                                        </div>
                                                    </div>

                                                </div>
                                            </div>

                                            <h4> User id
                                            </h4>
                                            <p class="mb-3">
                                                <span class="col-black font-16 font-600">
                                                    ist-pvt-ltd4</span>
                                            </p>
                                            <h4> Company Registration Number
                                            </h4>
                                            <p class="mb-3">
                                                <span class="col-black font-16 font-600">
                                                    COM00000124
                                                </span>
                                            </p>

                                            <h4> Ownership</h4>
                                            <p class="mb-3">
                                                <span class="col-black font-16 font-600">
                                                    Yes
                                                </span>
                                            </p>

                                            <h4> Owner name</h4>
                                            <p class="mb-3">
                                                <span class="col-black font-16 font-600">
                                                    British Agency

                                                </span>
                                            </p>


                                        </div>
                                        <div class="col-12 col-lg-6">
                                            <div class="mb-3 emailView">
                                                <h4>Email
                                                    Address</h4>
                                                <span id="spanProEmail" class="col-black font-16 font-600">
                                                    demo.healthcare@thestaffport.com
                                                </span>
                                            </div>

                                            <div class="mb-3 phoneView">
                                                <h4>Mobile
                                                    Number</h4>
                                                <span id="spanProMob" class="col-black font-16 font-600">Not
                                                    Available
                                                </span>
                                            </div>

                                            <h4> Number of Employees</h4>
                                            <p class="mb-3">
                                                <span class="col-black font-16 font-600">
                                                    50-100
                                                </span>
                                            </p>
                                            <h4>Industry</h4>
                                            <p class="mb-3">
                                                <span class="col-black font-16 font-600">
                                                    Healthcare
                                                </span>
                                            </p>


                                            <h4>Category</h4>
                                            <p class="mb-3">
                                                <span class="col-black font-16 font-600">
                                                    General Practice
                                                </span>
                                            </p>






                                        </div>

                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                </div>


            </div>



            <div class="panel-group">
                <div class="panel panel-primary  bdr-1">
                    <div class="panel-body pt-0 pb-0">
                        <div class="row">
                            <div class="col-sm-12 formbg p-0">
                                <div class="avt_bg d-flex justify-content-between align-items-center">
                                    <h3 class="mb-0"><b>Job Roles</b>
                                    </h3>




                                </div>
                                <div class="p-4 bdr_b1 View_General_info">
                                    <div class="row justify-content-start">
                                        <div class="col-12 col-lg-6">
                                            <div class="bxsaddow mb-2 p-2">

                                                <div class="p-3 bdr1 h4show cls-location-part">
                                                    <p class="mb-1"> <span class="wd--16px">User role</span> <span>:</span> <span id="Span5">Admin</span> </p>
                                                    <p class="mb-1"> <span class="wd--16px">Name</span> <span>:</span><span id="Span6">Aston Stoke </span>
                                                    </p>
                                                    <p class="mb-1"> <span class="wd--16px">Contact No. </span><span>:</span> <span id="Span7">122 2345 678</span> </p>
                                                    <p class="mb-1"> <span class="wd--16px">Email </span> <span>:</span><span id="Span8">amarendra@infospry.com</span> </p>

                                                </div>
                                            </div>

                                        </div>

                                        <div class="col-12 col-lg-6">
                                            <div class="bxsaddow mb-2 p-2">

                                                <div class="p-3 bdr1 h4show cls-location-part">
                                                    <p class="mb-1"> <span class="wd--16px">User role</span> <span>:</span> <span id="Span5">Admin</span> </p>
                                                    <p class="mb-1"> <span class="wd--16px">Name</span> <span>:</span><span id="Span6">Aston Stoke </span>
                                                    </p>
                                                    <p class="mb-1"> <span class="wd--16px">Contact No. </span><span>:</span> <span id="Span7">122 2345 678</span> </p>
                                                    <p class="mb-1"> <span class="wd--16px">Email </span> <span>:</span><span id="Span8">amarendra@infospry.com</span> </p>

                                                </div>
                                            </div>

                                        </div>
                                        <div class="col-12 col-lg-6">
                                            <div class="bxsaddow mb-2 p-2">

                                                <div class="p-3 bdr1 h4show cls-location-part">
                                                    <p class="mb-1"> <span class="wd--16px">User role</span> <span>:</span> <span id="Span5">Admin</span> </p>
                                                    <p class="mb-1"> <span class="wd--16px">Name</span> <span>:</span><span id="Span6">Aston Stoke </span>
                                                    </p>
                                                    <p class="mb-1"> <span class="wd--16px">Contact No. </span><span>:</span> <span id="Span7">122 2345 678</span> </p>
                                                    <p class="mb-1"> <span class="wd--16px">Email </span> <span>:</span><span id="Span8">amarendra@infospry.com</span> </p>

                                                </div>
                                            </div>

                                        </div>
                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                </div>


            </div>
        </>
    )
}
